...
div.vue-form-generator(v-if='schema != null')
  ...
  template(v-for='row in rows')
    div.row
      template(v-for='field in row.cols')
        div(:class='getGridColClasses(field, row)')
          form-group(v-if='fieldVisible(field)', :vfg="vfg", :field="field", :errors="errors", :model="model", :options="options", @validated="onFieldValidated", @model-updated="onModelUpdated")
</template>
...
export default {
  ...
  computed: {
    ...
    rows() {
      let res = [];
      if (this.schema && this.schema.grid && this.schema.grid.rows) {
        forEach(this.schema.grid.rows.slice(0), group => {
          res.push(group);
        });
      }
  ...
  methods: {
		getGridColClasses(col, row){
			const c = {"col": true};
			if(col.cols) {
				c["col-" + (row.size || "sm") +"-" + col.cols] = true;
      }
			return c;
		},
      ...
      if (!res && errors && errors.length > 0) {
        ...
        forEach(errors, err => {
          this.errors.push({
			...
			if(isFunction(this.validationService)){
				let result = this.validationService(this.model, {fields: [field.schema]});
    ...
    validate(isAsync = null) {
      ...
      let handleErrors = errors => {
				let externalErrors = [];
				if(isFunction(this.validationService)){
					let result = this.validationService(this.model, this.schema);
					console.log("EXTERNAL", result);
					forEach(result, error => {
						externalErrors.push(error);
					});
				}
        let formErrors = [].concat(externalErrors);
